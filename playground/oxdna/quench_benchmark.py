...
from pele.optimize.quench import lbfgs_cpp, fire
...
for coords in configurations[0:5]:
    ret = lbfgs_cpp(coords, pot.getEnergyGradient, tol=options.tol, M=options.M, maxstep=options.maxstep, maxErise=options.maxErise)
    energy = ret[1]
    fcalls = ret[3]
